...
import de.javabegin.micro.planner.todo.feign.UserFeignClient;
...
public class CategoryController {
    ...
    public ResponseEntity<Category> add(@RequestBody Category category) {
        ...
        // вызов МС с помощью RestTemplate
...
//        }
        ...
        if (result.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE) {
            return new ResponseEntity("Микросервис planner-users  не доступен", HttpStatus.I_AM_A_TEAPOT);
        }
        if (result.getBody() != null) {
            return ResponseEntity.ok(categoryService.add(category));
        }

        return new ResponseEntity("User id = '" + category.getUserId() + "' not found", HttpStatus.CONFLICT);
